...
import numpy as np
from util import __spectral_sync_utils, utils
import torchvision
...
class SpectralRPR(nn.Module):
    ...
    def forward_spectral(self, exp_rel_knn_ts, rel_knn_rots,
        ...
        for i in range(batch_size):
            rel_trans_mat = __spectral_sync_utils.compose_exp_rel_trans_mat(rel_query_ts[i * k:(i + 1) * k, :], exp_rel_knn_ts[i, :, :])
            abs_t, _ = __spectral_sync_utils.spectral_sync_trans(rel_trans_mat, exp_abs_knn_ts[i, :, :])
            abs_ts[i, :] = abs_t

            rel_rot_mat = __spectral_sync_utils.compose_rel_rot_mat(rel_query_quats[i * k:(i + 1) * k, :],
                                                                  rel_knn_rots[i, :, :])
            abs_rot, _ = __spectral_sync_utils.spectral_sync_rot(rel_rot_mat, abs_knn_rots[i])
